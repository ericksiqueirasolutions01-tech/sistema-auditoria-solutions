...
export const ConsultaProdutos: React.FC = () => {
  ...
  const handleExcluir = (id: number, serial: string) => {
    if (!isAdmin) return;
    if (window.confirm(`Tem certeza que deseja excluir o serial ${serial}? Esta ação será registrada no histórico de auditoria.`)) {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Busca avançada por serial, modelo, caixa, data ou status de lacre
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-black text-slate-800 uppercase">
                Editar Registro - Serial: {editando.serial}
              </h3>
              <button
