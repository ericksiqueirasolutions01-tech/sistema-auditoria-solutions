...
describe('GATE 9: Hardening do App Desktop e Segurança Localhost', () => {
  ...
  describe('1. Alinhamento de Versão Única (Gate 13.5)', () => {
    it('deve manter congruência estrita entre package.json, public/version.json e src/version.ts', () => {
      ...
      expect(pkgJson.version).toBe('1.3.2');
      ...
      expect(publicVerJson.versao).toBe('1.3.2');
      expect(publicVerJson.versaoCodigo).toBe(132);
    ...
    it('deve conter a versão 1.3.2 no instalador Windows (InstallerWizard.cs) e no app host (SistemaAuditoriaApp.cs)', () => {
      ...
      expect(installerCode).toContain('Versão 1.3.2');
      expect(installerCode).toContain('DisplayVersion", "1.3.2"');

      const appPath = path.join(rootDir, 'src-desktop', 'SistemaAuditoriaApp.cs');
      const appCode = fs.readFileSync(appPath, 'utf8');
      expect(appCode).toContain('1.3.2');
      expect(appCode).toContain('132');
    });
  });
